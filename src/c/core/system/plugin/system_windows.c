 ...
 *
 * The Asgard system-commands library, Windows plugin.
 *
...
// declare system (windows plugin) private functions
...
// define system (windows plugin) private functions
...
static int executeStat(char *filename, struct stat *statistics)
{
	...
	if(isLocal) {
		...
		system_fileSetNativeWindows(localFilename);
	}
...
static aboolean fileExists(char *filename)
{
	...
	system_fileExistsWindows(filename, &exists);

	return exists;
}


// define system (windows plugin) public functions
...
int system_fileDeleteWindows(char *filename)
{
	...
	rc = _unlink((const char *)localFilename);
...
int system_fileMoveWindows(char *sourceFilename, char *destFilename)
{
	...
	if(isLocal) {
		...
		system_fileSetNativeWindows(localSourceFilename);
		localDestFilename = strdup(destFilename);
		system_fileSetNativeWindows(localDestFilename);
	}
	...
	if((rc != 0) && (errno == EXDEV)) {
		result = system_fileCopyWindows(localSourceFilename, localDestFilename);
	}
	else if(rc != 0) {
		if(EXPLICIT_ERRORS) {
...
int system_fileCopyWindows(char *sourceFilename, char *destFilename)
{
	...
	if(fopen_s(&inFd, localSourceFilename, "rb") != 0) {
		if(EXPLICIT_ERRORS) {
	...
	if(fopen_s(&outFd, localDestFilename, "wb") != 0) {
		if(EXPLICIT_ERRORS) {
			fprintf(stderr, "[%s():%i] error - failed to open file '%s' for "
					"copy with '%s'.\n", __FUNCTION__, __LINE__,
					destFilename, strerror(errno));
	...
	inRc = (int)fread(block, SIZEOF_CHAR, blockLength, inFd);
	while(inRc > 0) {
		outRc = (int)fwrite(block, SIZEOF_CHAR, inRc, outFd);
		if(outRc <= 0) {
			break;
		}
		inRc = (int)fread(block, SIZEOF_CHAR, blockLength, inFd);
...
int system_dirChangeToWindows(char *dirname)
{
	...
	if(isLocal) {
		...
		system_fileSetNativeWindows(localDirname);
	}

	rc = _chdir(localDirname);
